...
<?php
	...
	if ($_POST) {
		...
		)
		{
			if (($_FILES["imagem"]["size"] < 5*1024*1024)){
				...
				else
				{
					...
					echo "<a href=\"../banner/g_".$imagem."\" target=\"blank\">".$imagem."</a><br>";
				}
			}
			else
			{
		...
		if ($_POST['acao']=='Inserir'){
			$sql = "insert into banner (imagem,obs) values ('".$imagem."','".$_POST["obs"]."');";
			mysql_query($sql, $link);
				if($sql){
			$erro = "Dados salvos com sucesso!";
				header("Location: ../adm/principal.php?pg=cadastro_banner");
			} 
			else
			{
		...
		$sql = "update banner set imagem='".$imagem."',obs='".$_POST["obs"]."' where (id=".$_POST["id"].");";
		//echo $sql;
		if($sql){
