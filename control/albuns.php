...
<?php
	...
	if ($_POST) {
		...
		if ($_POST['acao']=='Inserir'){
			$sql = "insert into albuns (imagem,titulo,data_emissao) values ('".$imagem."','".$_POST["titulo"]."','".$_POST["data_emissao"]."');";
			$reultado = mysql_query($sql, $link);
			echo $sql;
			if($reultado){
			$erro = "Dados salvos com sucesso!";
				header("Location: ../adm/principal.php?pg=cadastro_albuns");
			} 
			else
			{
		...
		$sql = "update imagem set albuns='".$imagem."',titulo='".$_POST["titulo"]."',data_emissao='".$_POST["data_emissao"]."' where (id=".$_POST["id"].");";
		echo $sql;
		$reultado = mysql_query($sql, $link);
		if($sql){
				$erro = "Dados salvos com sucesso!";
